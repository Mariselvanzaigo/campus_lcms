...
function getOrglist(parameter){
  ...
  if($( "#pagination-container-to" ).length > 0) {
    $('#pagination-container-to').pagination({
      dataSource: API_BASE_URL + 'parent/institute/list/'+parameter,
      locator: 'data',
...
$(document).on("click", "#cms-org_list li", function(){
  ...
  if(org_id != "" && org_id != null && org_id != undefined && org_institute_count){
    ...
    var outerHtml = '';
    $.ajax({
      url: API_BASE_URL + 'institute_list/?institute_id='+org_id,
      type: "GET",
      headers: {
        "Authorization": "Bearer " + getUserInfo().access_token,
        "Content-Type": "application/json"
      ...
      success:function(resp){
        if(resp){
            ...
            var header_name = resp.organization_name;
            if (resp.children.length > 0){
              has_child = "has_child";
              if(resp.children[0].name){
                header_name += " / "+resp.children[0].name
              }
            }
            $("#cms_institute_module_name_text").text(header_name);
...
function orgtoNoInstitute(org_id){
  ...
  $("#add_org_container_main").removeClass("d-none");
  $("#no_records_org_main_p").text("No Records have been found. Click the button below to create one.");
  $("#add_org_button_main").html(`<i class="fas fa-plus"></i> Add College`);
  $("#add_org_button_main").attr("data-n-linkto", "addinstitutecollege");
  ...
  $("#add_org_button_top").html(`<i class="fas fa-plus"></i> Add College`);
  $("#add_org_button_top").attr("data-n-linkto", "addinstitutecollege");
